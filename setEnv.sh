...
if [ ! $AUTH0_CLIENT_ID ] ; then
	export AUTH0_CLIENT_ID=`cat $DOCKER_CONFIG_DIR/httpd/picsureui_settings.json | grep client_id | cut -d ":" -f 2 | sed 's/\",*//g'`
fi

if [ ! $AUTH0_TENANT ] ; then
	export AUTH0_TENANT=`cat $DOCKER_CONFIG_DIR/httpd/picsureui_settings.json | grep auth0domain | cut -d ":" -f 2 | sed 's/\",*//g'`
fi
...
sed -i "s/__AUTH0_CLIENT_ID__/$AUTH0_CLIENT_ID/g" ui/src/main/webapp/picsureui/settings/settings.json
sed -i "s/__AUTH0_DOMAIN__/$AUTH0_TENANT/g" ui/src/main/webapp/picsureui/settings/settings.json
